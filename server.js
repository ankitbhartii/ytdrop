...
// Relaxed rate limit — fine for local personal use
const limiter = rateLimit({
  windowMs: 1 * 60 * 1000,  // 1 minute window
  max: 200,                  // 200 requests per minute — effectively no limit locally
...
app.get("/api/info", (req, res) => {
  ...
  const proc = spawn(bin, ["--dump-json", "--no-playlist", "--no-warnings", ytUrl]);
  let stdout = "";
  let stderr = "";
...
app.post("/api/convert", (req, res) => {
  ...
  if (format === "mp3") {
    ...
    // ── MP4: download best video+audio, re-encode audio to AAC for compatibility
    const heightMap = { "1080p": 1080, "720p": 720, "480p": 480, "360p": 360 };
    const height = heightMap[quality] || 720;
    args = [
      // Prefer h264 video + m4a audio (most compatible, no re-encode needed)
      // Falls back to any best video+audio if those aren't available
      "-f", `bestvideo[vcodec^=avc][height<=${height}]+bestaudio[ext=m4a]/bestvideo[height<=${height}]+bestaudio/best`,
      "--merge-output-format", "mp4",
      "--no-playlist",
      "--no-warnings",
      "--newline",
      // Re-encode audio to AAC if it's not already — this is the key fix for silent videos
  ...
  proc.on("close", (code) => {
    ...
    if (code !== 0) {
      const err = stderrBuf.includes("Sign in") ? "This video requires sign-in or is age-restricted."
        ...
        : `Conversion failed (exit ${code}). Check VS Code terminal for details.`;
      jobs[jobId].status = "error";
    ...
    if (outputFile && fs.existsSync(outputFile)) {
      ...
      console.error(`[convert] File not found. Downloads dir:`, fs.readdirSync(DOWNLOADS_DIR));
      jobs[jobId].status = "error";
      jobs[jobId].error = "File not found after conversion. Check terminal for details.";
    }
  });
});
